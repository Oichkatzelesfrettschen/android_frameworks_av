...
CameraClient::CameraClient(const sp<CameraService>& cameraService,
        ...
        const std::string& cameraIdStr, int api1CameraId, int cameraFacing,
        int sensorOrientation, int servicePid, int rotationOverride, bool sharedMode):
        Client(cameraService, cameraClient, attributionAndPermissionUtils, clientAttribution,
                callingPid, false /*systemNativeClient - no ndk for api1*/, cameraIdStr,
                api1CameraId, cameraFacing, sensorOrientation, servicePid, rotationOverride,
                sharedMode),
...
void CameraClient::startStreamingOpsIfNeeded() {
    if (mStreamingOpsActive) return;
    if (startCameraStreamingOps() == OK) {
        mStreamingOpsActive = true;
    }
}

void CameraClient::finishStreamingOpsIfNeeded() {
    if (!mStreamingOpsActive) return;
    finishCameraStreamingOps();
    mStreamingOpsActive = false;
}
...
status_t CameraClient::initialize(sp<CameraProviderManager> manager,
    ...
    // The legacy provider names HALv1 devices by the integer index of camera_module_t,
    // and the callback cookie below carries that index.
    int halCameraId;
    if (!base::ParseInt(mCameraIdStr, &halCameraId, 0)) {
        ALOGE("%s: Camera %s: HALv1 camera IDs must be non-negative integers", __FUNCTION__,
                mCameraIdStr.c_str());
        return BAD_VALUE;
    }

    // Verify ops permissions
    res = notifyCameraOpening();
    if (res != OK) {
        return res;
    }

    mHardware = new CameraHardwareInterface(mCameraIdStr.c_str());
    res = mHardware->initialize(manager);
    if (res != OK) {
        ALOGE("%s: Camera %d: unable to initialize device: %s (%d)",
                __FUNCTION__, mCameraId, strerror(-res), res);
        mHardware.clear();
        // disconnect() returns early without mHardware, so undo notifyCameraOpening() here.
        notifyCameraClosing();
        return res;
    }
    ...
    mHardware->setCallbacks(notifyCallback,
            ...
            reinterpret_cast<void *>(static_cast<uintptr_t>(halCameraId)));
...
status_t CameraClient::checkPid() const {
    int callingPid = getCallingPid();
    if (callingPid == mCallingPid) return NO_ERROR;

    ALOGW("attempt to use a locked camera from a different process"
         " (old pid %d, new pid %d)", mCallingPid, callingPid);
    return EBUSY;
}
...
status_t CameraClient::connect(const sp<hardware::ICameraClient>& client) {
    ...
    if (mCallingPid != 0 && checkPid() != NO_ERROR) {
...
binder::Status CameraClient::disconnect() {
    ...
    if (callingPid != mCallingPid && callingPid != mServicePid) {
    ...
    finishStreamingOpsIfNeeded();
    mCameraServiceProxyWrapper->logIdle(mCameraIdStr,
            /*requestCount*/0, /*resultErrorCount*/0, /*deviceError*/false,
            /*userTag*/"", /*videoStabilizationMode*/0, /*usedUltraWide*/false,
            /*usedZoomOverride*/false, /*mostRequestedFpsRange*/{0, 0},
            /*streamStats*/{});
...
status_t CameraClient::setPreviewTarget(const sp<SurfaceType>& target) {
    LOG1("setPreviewTarget(%p) (pid %d)", target.get(), getCallingPid());

    sp<IBinder> binder;
    sp<ANativeWindow> window;
    if (target != nullptr) {
        sp<IGraphicBufferProducer> bufferProducer = flagtools::surfaceTypeToIGBP(target);
...
status_t CameraClient::setPreviewCallbackTarget(const sp<SurfaceType>& target) {
    (void)target;
...
status_t CameraClient::startPreviewMode() {
    ...
    if (result == NO_ERROR) {
        startStreamingOpsIfNeeded();
        mCameraServiceProxyWrapper->logActive(mCameraIdStr, /*maxPreviewFps*/0.f);
    }
    return result;
}
...
sp<CameraClient> CameraClient::getClientFromCookie(void* user) {
    std::string cameraId = std::to_string(static_cast<int>(reinterpret_cast<uintptr_t>(user)));
...
status_t CameraClient::setVideoTarget(const sp<SurfaceType>& /*target*/) {
...
void CameraClient::notifyClientSharedAccessPriorityChanged(bool /*primaryClient*/) {
}
...
void CameraClient::setStreamUseCaseOverrides(const std::vector<int64_t>& /*useCaseOverrides*/) {
}
