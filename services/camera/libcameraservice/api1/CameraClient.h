...
public:
    ...
    // Client features added after AOSP dropped HALv1; a Camera1 HAL cannot honor them.
    ...
    virtual bool            supportsCameraMute() override;
    virtual status_t        setCameraMute(bool enabled) override;
    virtual status_t        setCameraServiceWatchdog(bool enabled) override;
    ...
    virtual status_t        stopInjection() override;
...
private:
    ...
    // HAL1 has no device-idle callback, so keep the streaming ops paired here.
    bool mStreamingOpsActive = false;
    void startStreamingOpsIfNeeded();
    void finishStreamingOpsIfNeeded();
    ...
    // The HAL callback cookie carries the numeric HAL camera ID, which keys
    // CameraService::mActiveClientManager.
